#https://pypi.org/project/lzstring/#files <- all credits to him, i added decompressCustom & compressToCustom
...
class LZString(object):
    ...
    @staticmethod
    def compressToCustom(uncompressed, key):
        if uncompressed is None:
            return ""
        res = _compress(uncompressed, 6, lambda a: key[a])
        # To produce valid Base64
        end = len(res) % 4
        if end > 0:
            res += "=" * (4 - end)
    ...
    @staticmethod
    def decompressCustom(compressed, key):
        if compressed is None:
            return ""
        if compressed == "":
            return None
        return _decompress(len(compressed), 32, lambda index: getBaseValue(key, compressed[index]))
